...
<AccordionGroup>
  ...
  <Accordion title="--dangerously-force-unsafe-install">
    ...
    If a plugin you published on ClawHub is blocked by a registry scan, use the publisher steps in [ClawHub publishing](/clawhub/publishing).
...
`doctor` reports plugin load errors, manifest/discovery diagnostics, compatibility notices, and stale plugin config references. When everything is clean it prints `No plugin issues detected.` If only stale config references remain after install-tree checks pass, it reports that no plugin install-tree issues were detected and that configuration warnings remain.
